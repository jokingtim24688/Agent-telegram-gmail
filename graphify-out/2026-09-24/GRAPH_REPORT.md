...
Cohesion: 0.07
Nodes (34): argparse, _env(), _env_int(), load(), Path, Settings from desktop/.env plus small runtime state in desktop/data/state.json., Values learned at runtime that must survive restarts (ids, chosen model)., Human-readable setup gaps, shown on the dashboard's checklist. (+26 more)

### Community 1 - "core.py"
Cohesion: 0.10
Nodes (24): asyncio, base64, collections, The AI's brain: describes frames, and chats with the owner using two tools., Wires the pieces together and holds the live status the dashboard shows., Gmail backup path. Each device has its own Gmail account: glasses Gmail…, OllamaError, RuntimeError (+16 more)
...
### Community 3 - "Core"
Cohesion: 0.07
Nodes (15): Agent, Put the frame's description into the chat history, so follow-ups like 'how much…, Core, Path, Telegram first. If that fails, fall back to email so nothing gets lost., Per-component health: state is ok | warn | error | off., Tool + /look + dashboard button: make the glasses take a photo now., If this frame answers a /look, return that question (once). (+7 more)
...
## Knowledge Gaps
- **28 isolated node(s):** `seen`, `wifiJson`, `tgToken`, `tgChat`, `tgFrom` (+23 more)
