# Core

> God node · 19 connections · `desktop/glasses_agent/core.py`

**Community:** [Core](Core.md)

## Connections by Relation

### calls
- run() `EXTRACTED`

### contains
- [core.py](core.py.md) `EXTRACTED`

### imports
- __main__.py `EXTRACTED`

### method
- .__init__() `EXTRACTED`
- .handle_frame() `EXTRACTED`
- .model() `EXTRACTED`
- .notify_owner() `EXTRACTED`
- ._question_for_frame() `EXTRACTED`
- .chat() `EXTRACTED`
- .watch_ollama() `EXTRACTED`
- .glasses_heard() `EXTRACTED`
- .request_snapshot() `EXTRACTED`

### uses
- [Settings](Settings.md) `INFERRED`
- [State](State.md) `INFERRED`
- [LLM](LLM.md) `INFERRED`
- [Agent](Agent.md) `INFERRED`
- VisualMemory `INFERRED`
- [Capture](Capture.md) `INFERRED`
- OllamaError `INFERRED`

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*